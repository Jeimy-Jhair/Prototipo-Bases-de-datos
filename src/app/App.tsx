...
//Importamos las funciones que vamos a utilizar en la API
import {
    obtenerClientes,
    crearCliente,
    actualizarCliente,
  ...
  return (
    <div>
        ...
        <PageHeader
            ...
            actions={ <>
                    <Btn variant="save" icon={<Plus size={14} />} onClick={openNew} >
                        Nueva Sucursal
                    </Btn>
                </>
            }
                        ...
                        <tr
                            ...
                            <Td>
                                <div className="flex items-center gap-1.5">
                                    ...
                                    <span className="font-medium"> {s.ciudad}
                                    </span>
                                </div>
                            </Td>
                            <Td>
        ...
        {(modal === "new" || modal === "edit") && (
            ...
            >
                <div className="grid grid-cols-2 gap-3">
                <Input label="Código IATA" value={form.codigo_iata} onChange={v => setForm({ ...form,  codigo_iata: v }) } disabled={modal === "edit"} required />
                <Input label="Ciudad" value={form.ciudad} onChange={v => setForm({ ...form, ciudad: v}) }required/>
                </div>
            </Modal>
        )}
        {confirmId && (
            <ConfirmDialog message={`¿Eliminar la sucursal ${confirmId}?`} onConfirm={() => remove(confirmId)} onCancel={() => setConfirmId(null)}/>
        )}
    </div>
);
}
...
function VehiculosIdScreen({ data, setData }: { data: VehiculoId[]; setData: (d: VehiculoId[]) => void }) {
  ...
  const [modal, setModal] = useState<"" | "new" | "edit">("");
  const [confirmId, setConfirmId] = useState<string | null>(null);
  const [form, setForm] = useState<VehiculoId>({ placa: "", marca: "" });
  const PER_PAGE = 6;
  ...
  const save = async () => {
    ...
    try {
      if (modal === "new") {
        await crearVehiculoIdentificacion({ placa: form.placa, marca: form.marca });
      } else {
        await actualizarVehiculoIdentificacion({ placa: form.placa, marca: form.marca });
      }
    ...
    <div>
      ...
      {(modal === "new" || modal === "edit") && (
        <Modal title={modal === "new" ? "Nuevo Vehículo" : "Editar Vehículo"} onClose={() => setModal("")}
          footer={<><Btn variant="ghost" onClick={() => setModal("")}>Cancelar</Btn><Btn variant="save" icon={<Check size={14} />} onClick={save}>Guardar</Btn></>}>
          <div className="grid grid-cols-2 gap-3">
            <Input label="Placa" value={form.placa} onChange={v => setForm({ ...form, placa: v })} required disabled={modal === "edit"} />
...
function VehiculosTecScreen({ data, setData }: { data: VehiculoTec[]; setData: (d: VehiculoTec[]) => void; activeBranch: Branch }) {
  ...
  const [modal, setModal] = useState<"" | "new" | "edit">("");
  const [confirmId, setConfirmId] = useState<string | null>(null);
  const [form, setForm] = useState<VehiculoTec>({ placa: "", anio: 2022, capacidad: "", codigo_iata: "UIO" });
  const PER_PAGE = 6;
  useEffect(() => { setFilter("ALL"); setPage(1); }, ["ALL"]);
...
export default function App() {
  ...
  async function cargarRepartidores() {
    ...
    try {
      ...
      async function cargarVehiculosId() {
    try {
      const datos = await obtenerVehiculosIdentificacion();
